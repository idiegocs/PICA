...
public class OrdenController implements Serializable {
    ...
    @EJB
    private com.kallsony.facade.service.CancelarServiceFacade ejbCancelarFacade;
    ...
    final static org.apache.log4j.Logger log = org.apache.log4j.Logger.getLogger(OrdenController.class.getName());
    ...
    public void cancelarOrden() {
        ...
        String comentario = "";
        try {
            if (selected != null && selected.getIdestadoorden()!=null && (selected.getIdestadoorden().getIdestadoorden().intValue()!=7)) {
                urlServ = FacesContext.getCurrentInstance().getExternalContext().getInitParameter("kallsony.ipservcancelar");
                log.info("Url serv cancelar " + urlServ);
                FacesContext fc = FacesContext.getCurrentInstance();
                ExternalContext externalContext = fc.getExternalContext();
                usuario = externalContext.getUserPrincipal().getName();
                log.info("Usuario " + usuario);
                comentario = "Cancelada Por administrador";
                cancelo = ejbCancelarFacade.cancelarOrden(urlServ, selected.getIdorden().intValue(), usuario, comentario);

                if (cancelo) {
                    items =null;
                    JsfUtiles.addSuccessMessage("Se Cancelo la Orden...");
                } else {
                    JsfUtiles.addErrorMessage("No se pudo Cancelar la Orden - Intente Nuevamente.");
                }
            } else {
                JsfUtiles.addSuccessMessage("Seleccione Una Orden (No Cancelada)...");
            }

        } catch (Exception e) {
            log.error("Error al cancelar ", e);
            JsfUtiles.addErrorMessage("Error al Cancelar Orden - Intente Nuevamente.");
        }
    }
